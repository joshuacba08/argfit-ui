...
![Status](https://img.shields.io/badge/status-2.2.0-2599D5)
...
ArgFit UI is aligned on the stable `2.2.0` production contract. Publishable packages use exact `2.2.0` internal peers, public npm metadata and the `latest` dist-tag.
...
The workspace and publishable packages are aligned on `2.2.0`. The root workspace remains private. Publishable `@argfit-ui/*` packages, including `@argfit-ui/chart-runtime` and `@argfit-ui/mcp`, are MIT licensed and prepared for public npm distribution with the `latest` dist-tag, while production tarballs remain available for manual verification under `dist/production-tarballs/`.
