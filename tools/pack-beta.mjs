...
const BETA_VERSION = '0.1.0-beta.0';
const repoRoot = fileURLToPath(new URL('..', import.meta.url));
const isDryRun = process.argv.includes('--dry-run');
const tarballDestination = resolve(repoRoot, 'dist', 'beta-tarballs');
...
function validateManifest(packageDefinition, manifest) {
  ...
  if (manifest.version !== BETA_VERSION) {
    fail(`${packageDefinition.name} must be version ${BETA_VERSION}, found ${manifest.version}.`);
  }

  if (manifest.private === true) {
    fail(`${packageDefinition.name} must not be private for beta packaging.`);
  }
  ...
  for (const internalPeerName of packageDefinition.internalPeers) {
    if (peerDependencies[internalPeerName] !== BETA_VERSION) {
      fail(
        `${packageDefinition.name} peer ${internalPeerName} must be ${BETA_VERSION}, found ${peerDependencies[internalPeerName]}.`,
      );
    }
  }
...
function validatePackEntry(packageDefinition, npmPackEntry) {
  if (!npmPackEntry || npmPackEntry.name !== packageDefinition.name || npmPackEntry.version !== BETA_VERSION) {
    fail(`${packageDefinition.name} npm pack metadata does not match the beta manifest.`);
  }
