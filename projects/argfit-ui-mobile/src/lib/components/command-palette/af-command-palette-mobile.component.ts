...
@Component({
  selector: 'af-command-palette-mobile',
  imports: [
    AfEscapeKeyDirective,
    AfFocusInitialDirective,
    AfFocusTrapDirective,
  ...
  styleUrl: './af-command-palette-mobile.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-command-palette-mobile',
    '[attr.data-open]': 'open() ? "" : null',
  },
})
export class AfCommandPaletteMobileComponent implements OnDestroy {
  private readonly document = inject(DOCUMENT);
  private readonly isBrowser = isPlatformBrowser(inject(PLATFORM_ID));
  private readonly searchInput = viewChild<ElementRef<HTMLInputElement>>('searchInput');
  private readonly instanceId = ++nextAfCommandPaletteMobileId;
  ...
  readonly groups = input<readonly AfCommandPaletteMobileRenderGroup[]>([]);
  ...
  protected groupLabelId(group: AfCommandPaletteMobileRenderGroup): string {
