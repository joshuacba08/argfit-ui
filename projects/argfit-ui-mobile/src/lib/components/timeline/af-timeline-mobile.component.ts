...
type AfTimelineMobileState = 'ready' | 'loading' | 'empty' | 'error';

@Component({
  selector: 'af-timeline-mobile',
  imports: [AfBadgeMobileComponent, AfIconComponent, NgTemplateOutlet],
  templateUrl: './af-timeline-mobile.component.html',
  styleUrl: './af-timeline-mobile.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-timeline-mobile',
    '[attr.data-density]': 'density()',
    '[attr.data-state]': 'state()',
  },
})
export class AfTimelineMobileComponent {
  ...
  protected readonly state = computed<AfTimelineMobileState>(() => {
    if (this.error()) {
      return 'error';
    }
