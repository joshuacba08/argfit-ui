...
type AfOrderListDesktopState = 'ready' | 'loading' | 'empty' | 'error';

@Component({
  selector: 'af-order-list-desktop',
  imports: [AfBadgeDesktopComponent, AfIconComponent, NgTemplateOutlet],
  templateUrl: './af-order-list-desktop.component.html',
  styleUrl: './af-order-list-desktop.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-order-list-desktop',
    '[attr.data-density]': 'density()',
    '[attr.data-state]': 'state()',
    '[attr.data-disabled]': 'disabled() ? "" : null',
  },
})
export class AfOrderListDesktopComponent {
  ...
  protected readonly state = computed<AfOrderListDesktopState>(() => {
    if (this.error()) {
      return 'error';
    }
  ...
  private focusSibling(currentTarget: HTMLButtonElement, direction: 'next' | 'previous'): void {
    const list = currentTarget.closest('.af-order-list-desktop__list');
    const items = Array.from(list?.querySelectorAll('.af-order-list-desktop__item') ?? []).filter(
  ...
  private focusEdge(edge: 'start' | 'end'): void {
    const items = Array.from(document.querySelectorAll('.af-order-list-desktop__item')).filter(
      (item): item is HTMLButtonElement => item instanceof HTMLButtonElement && !item.disabled,
    );
