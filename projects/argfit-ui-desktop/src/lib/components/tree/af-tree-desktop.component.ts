...
type AfTreeDesktopState = 'ready' | 'loading' | 'empty' | 'error';

@Component({
  selector: 'af-tree-desktop',
  imports: [AfBadgeDesktopComponent, AfIconComponent, NgTemplateOutlet],
  templateUrl: './af-tree-desktop.component.html',
  styleUrl: './af-tree-desktop.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-tree-desktop',
    '[attr.data-density]': 'density()',
    '[attr.data-state]': 'state()',
    '[attr.data-disabled]': 'disabled() ? "" : null',
  },
})
export class AfTreeDesktopComponent {
  ...
  protected readonly state = computed<AfTreeDesktopState>(() => {
    if (this.error()) {
      return 'error';
    }
  ...
  private visibleNodeButtons(): HTMLButtonElement[] {
    return Array.from(this.hostRef.nativeElement.querySelectorAll('.af-tree-desktop__label')).filter(
