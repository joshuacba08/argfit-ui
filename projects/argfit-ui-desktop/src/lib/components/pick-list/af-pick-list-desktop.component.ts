...
type AfPickListDesktopState = 'ready' | 'loading' | 'empty' | 'error';

@Component({
  selector: 'af-pick-list-desktop',
  imports: [AfBadgeDesktopComponent, AfIconComponent, NgTemplateOutlet],
  templateUrl: './af-pick-list-desktop.component.html',
  styleUrl: './af-pick-list-desktop.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-pick-list-desktop',
    '[attr.data-density]': 'density()',
    '[attr.data-state]': 'state()',
    '[attr.data-disabled]': 'disabled() ? "" : null',
  },
})
export class AfPickListDesktopComponent {
  ...
  protected readonly state = computed<AfPickListDesktopState>(() => {
    if (this.error()) {
      return 'error';
    }
  ...
  private focusSibling(list: AfPickListListKey, currentTarget: HTMLButtonElement, direction: 'next' | 'previous'): void {
    const items = Array.from(this.hostRef.nativeElement.querySelectorAll(`.af-pick-list-desktop__item[data-list="${list}"]`)).filter(
