...
@Component({
  selector: 'af-kanban-desktop',
  imports: [CdkDrag, CdkDropList, CdkDropListGroup, NgTemplateOutlet],
  templateUrl: './af-kanban-desktop.component.html',
  styleUrl: './af-kanban-desktop.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    class: 'af-kanban-desktop',
    '[class]': 'hostClasses()',
...
export class AfKanbanDesktopComponent {
  ...
  protected readonly activeDropColumnId = signal<string | null>(null);
  protected readonly liveMessage = signal('');
  protected readonly hostClasses = computed(() => ['af-kanban-desktop', `af-kanban-desktop--${this.density()}`].join(' '));
  protected readonly connectedDropListIds = computed(() => this.columns().map((column) => this.dropListId(column.id)));
  protected readonly isFiltering = computed(() => !!this.activeFilter() && this.activeFilter() !== 'all');
  ...
  protected onDrop(event: CdkDragDrop<readonly AfKanbanCard[]>, targetColumn: AfKanbanColumn): void {
    if (this.dropDisabled(targetColumn)) {
      return;
    }

    const draggedCard = event.item.data as AfKanbanCard | undefined;
    const sourceColumnId = draggedCard ? this.resolveCardColumnId(draggedCard) : this.extractColumnId(event.previousContainer.id);
    if (!draggedCard || !sourceColumnId) {
      return;
    }

    const moveEvent = this.buildMoveEvent(
      draggedCard,
      sourceColumnId,
      targetColumn.id,
      event.previousIndex,
    ...
    this.emitMove(moveEvent);
  }

  protected onDropListEntered(columnId: string): void {
    this.activeDropColumnId.set(columnId);
  }

  protected onDropListExited(columnId: string): void {
    if (this.activeDropColumnId() === columnId) {
      this.activeDropColumnId.set(null);
    }
  }
  ...
  protected dropDisabled(column: AfKanbanColumn): boolean {
    return this.disabled() || this.readonly() || column.disabled || this.isFiltering();
  }

  protected cardsForColumn(columnId: string): readonly AfKanbanCard[] {
  ...
  protected columnAccent(column: AfKanbanColumn): string | null {
    return column.accentColor ?? null;
  }
  ...
  protected dropListId(columnId: string): string {
    return `af-kanban-desktop-drop-${columnId}`;
  }
  ...
  private extractColumnId(dropListId: string): string | undefined {
    return dropListId.replace('af-kanban-desktop-drop-', '') || undefined;
  }

  private emitMove(moveEvent: AfKanbanMoveEvent): void {
