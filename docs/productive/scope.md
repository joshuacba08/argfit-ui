...
This document defines the frozen public scope for `1.0.0`.

ArgFit UI `1.0.0` is not a broader beta. It is the first semver-major contract where the public barrels, recommended usage path and support expectations are intentionally frozen.
...
| `out-of-1.0` | Planned or historical surfaces that are not part of the `1.0.0` contract because they are not publicly exported or are intentionally outside the recommended path. |

Every public export included in the current package barrels must belong to one of the `1.0-*` categories above. There is no `experimental` public category in the `1.0.0` contract.
...
This is how the former beta and Beta+ experimental surface is resolved for `1.0.0`: promoted exports stay public and frozen, renderer-specific exports stay isolated in renderer packages, and backlog ideas that never shipped stay outside the `1.0.0` contract.
...
The adaptive barrel is the canonical application contract for `1.0.0`.
...
Teams moving from beta or Beta+ to `1.0.0` should:
...
- align their internal wrappers with the frozen `1.0.0` names and semantics documented in the productive API inventory