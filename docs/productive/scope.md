# ArgFit UI 1.5.0 Scope

## Status

This document defines the stable public scope for `1.5.0`.

ArgFit UI `1.5.0` extends the stable 1.x contract while keeping public barrels, the recommended usage path and support expectations explicit.
...
| `out-of-1.0` | Planned or historical surfaces that are not part of the `1.5.0` contract because they are not publicly exported or are intentionally outside the recommended path. |

Every public export included in the current package barrels must belong to one of the `1.0-*` categories above. There is no `experimental` public category in the `1.5.0` contract.
...
This is how the former beta and Beta+ experimental surface remains resolved for `1.5.0`: promoted exports stay public, renderer-specific exports stay isolated in renderer packages, and backlog ideas that never shipped stay outside the stable contract.
...
The adaptive barrel is the canonical application contract for `1.5.0`.
...
Teams moving from beta or Beta+ to `1.5.0` should:
...
- align their internal wrappers with the stable `1.5.0` names and semantics documented in the productive API inventory
