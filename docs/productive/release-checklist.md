...
Use this checklist before tagging or publishing ArgFit UI `1.3.4` production packages.

## Version And Metadata

- Confirm the root `package.json` version is `1.3.4`.
- Confirm every publishable `projects/argfit-ui-*/package.json` version is `1.3.4`.
- Confirm every internal `@argfit-ui/*` peer dependency is pinned to `1.3.4`.
...
- Confirm `CHANGELOG.md` contains the `1.3.4` production entry.
- Confirm `docs/productive/release-notes-1.3.4.md` describes the shipped release.
...
```bash
git tag -a v1.3.4 -m "ArgFit UI 1.3.4"
...
After publish, verify npm shows `@argfit-ui/core`, `@argfit-ui/primitives`, `@argfit-ui/desktop`, `@argfit-ui/mobile` and `@argfit-ui/adaptive` on version `1.3.4` under the `latest` dist-tag.
