...
Use this checklist before tagging or publishing ArgFit UI `2.2.0` production packages.

## Version And Metadata

- Confirm the root `package.json` version is `2.2.0`.
- Confirm every publishable `projects/argfit-ui-*/package.json` version is `2.2.0`.
- Confirm every internal `@argfit-ui/*` peer dependency is pinned to `2.2.0`.
...
- Confirm `CHANGELOG.md` contains the `2.2.0` production entry.
- Confirm `docs/productive/release-notes-2.2.0.md` describes the shipped release.
- Confirm `docs/productive/migration-1-to-2.md` documents the chart import break.
...
```bash
git tag -a v2.2.0 -m "ArgFit UI 2.2.0"
...
- workflow trigger is `v2.*.*` or a validated manual dispatch
...
- workflow publishes all seven dist packages with `npm publish --tag latest --access public`

After publish, verify npm shows `@argfit-ui/core`, `@argfit-ui/primitives`, `@argfit-ui/chart-runtime`, `@argfit-ui/desktop`, `@argfit-ui/mobile`, `@argfit-ui/adaptive` and `@argfit-ui/mcp` on version `2.2.0` under the `latest` dist-tag.
