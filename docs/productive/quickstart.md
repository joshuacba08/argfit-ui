...
This guide describes the consumer posture for the frozen `1.2.0` ArgFit UI contract.

The repository now validates stable `1.2.0` package metadata and production tarballs through the production release gate.
...
When `1.2.0` is published, keep every ArgFit package aligned to the exact same release:

```bash
pnpm add @argfit-ui/core@1.2.0
pnpm add @argfit-ui/primitives@1.2.0
pnpm add @argfit-ui/desktop@1.2.0 @argfit-ui/mobile@1.2.0
pnpm add @argfit-ui/adaptive@1.2.0
```
...
- [Productive release notes](./release-notes-1.2.0.md)
