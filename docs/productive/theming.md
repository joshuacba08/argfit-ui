...
ArgFit UI `1.0.0` remains token-driven. Consumers configure the runtime once, then use semantic `Af*` components from `@argfit-ui/adaptive`.
...
The productive contract ships with:
...
If you need token-aware utilities, import token names from `@argfit-ui/core` instead of hardcoding vendor selectors or copying renderer internals.
...
## Productive Theming Rules

- Prefer adaptive imports in application code.
- Treat PrimeNG and Ionic selectors as internal implementation details.
- Override tokens or build app-level wrappers instead of styling renderer internals.
- Keep app-level utility layers small and semantic.
- Validate visual changes with the same production gate that protects the package contract.
