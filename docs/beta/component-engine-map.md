...
- Wave 3 current status: `AfDataView`, `AfPaginator`, `AfOrderList`, `AfPickList`, `AfTimeline`, `AfTree`, `AfTreeTable` and `AfVirtualScroller` now have adaptive, desktop and mobile slices with focused specs and showcase coverage.
...
| `AfTreeTable` | `TreeTable` / grouped cards or list, not table shrink | Beta+ current HU-032 slice implemented |
| `AfVirtualScroller` | `VirtualScroller` or CDK virtual scroll / CDK virtual scroll or paginated fallback | Beta+ current HU-032 slice implemented with explicit fixed-height window contract |
